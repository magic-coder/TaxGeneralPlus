...
static void g_l3vpnStatusCallback(L3vpnStatus status) {
    id<SangforSDKDelegate> delegate = [AuthHelper getGlobalDelegate];
    if (delegate!=nil && [delegate respondsToSelector:@selector(onL3VPNStatusCallback:)]) {
        [delegate onL3VPNStatusCallback:status];
    }
}
...
- (BOOL )getIPAddress:(NSString*) hostname withAddr:(uint32_t *)addr{
    Boolean result;
    CFHostRef hostRef;
    ...
    if ([hostname rangeOfString:@"https://"].location != NSNotFound) {
    ...
    if (result == TRUE) {
        ...
        if (remoteAddr != NULL) {
            *addr = remoteAddr->sin_addr.s_addr;
            return YES;
        }
    }
...
- (void)init:(SdkMode)mode host:(NSString *)host port:(short)port delegate:(id <SangforSDKDelegate>)delegate {
    ...
    if (![self getIPAddress:host withAddr:&vpnAddr]) {
        vpnAddr = INADDR_NONE;
    }
    /*
    ...
    if (mode == EasyApp) {
        ...
        ssl_vpn_init_l3vpn(g_callBack,vpnAddr,vpnPort);
    }
...
-(int) setAuthParam:(NSString *)key param:(NSString*)value
{
    ...
    if (!strncmp(charKey,CERT_AUTH_SLECT_AUTH,sizeof(CERT_AUTH_SLECT_AUTH))) {
        snprintf(tmp,sizeof(tmp),"%p",g_selectCertCallBack);
        charValue = tmp;
    } else {
...
- (void) queryL3VPNStatus {
    ssl_vpn_query_l3vpn_status(g_l3vpnStatusCallback);
}
