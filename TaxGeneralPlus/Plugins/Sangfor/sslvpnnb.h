...
extern "C"
{
    ...
    /**
     * @功能：初始化l3vpn服务信息，包括初始化的回调函数，VPN服务器IP地址，端口号
     * @param callback 回调函数指针，初始化为nil时，表示不设置回调函数
     * @param host [in] VPN服务器地址，IPd地址或者为域名
     * @param port [in] VPN服务器端口，注意为网络序
     * return 0表示初始化成功，－1表示失败
     **/
    int ssl_vpn_init_l3vpn(VPN_CALL_BACK callback,uint32_t host,uint16_t port);
    ...
    /**
     ...
     @param callback 回调函数
     */
    void ssl_vpn_query_l3vpn_status(L3VPN_STATUS_CALLBACK callback);
     ...
     * @param value_length [in/out] value 的长度,by Bytes。返回实际长度
	 *
    ...
    /**
     * @功能 获取短信验证 再次发送倒计时
     * @return 返回短信验证 再次发送倒计时
     */
    const char *ssl_get_sms_countdown(char *buffer, int buffer_len);
     ...
     *  获取本地保存的rclist
     */
    NSString* ssl_get_local_rclist(bool saveRclist);
    
    /**
     *  获取服务端密码策略失败原因
     */
    NSString* ssl_get_password_safe_policy_prompt(const char* msg);
     ...
     */
    const char* ssl_get_localRclist(bool saveRclist);
