...
static const CGFloat SpringTreshold = 120;
...
@property (nonatomic,weak) IBOutlet NSLayoutConstraint *skyHeightConstraint;

@property (weak, nonatomic) IBOutlet NSLayoutConstraint *buildingsTopConstraint;
...
-(void)awakeFromNib{
    ...
    [self.skyHeightConstraint setConstant:200+HEIGHT_STATUS];
...
-(void)calculateShift{
    ...
    if(self.scrollView.contentOffset.y <= -DefaultHeight - HEIGHT_STATUS){
        
        if(self.scrollView.contentOffset.y < -SpringTreshold-HEIGHT_STATUS){
            
            [self.scrollView setContentOffset:CGPointMake(0.f, -SpringTreshold-HEIGHT_STATUS)];
        }
        [self scaleItems];
...
-(void)setupSunHeightAboveHorisont{
    ...
    CGFloat sunYCoordinate = SunBottomPoint - (sunWay / (100+HEIGHT_STATUS)) * shiftInPercents;
    if(DEVICE_SCREEN_INCH_5_8)
        [self.sunTopConstraint setConstant:sunYCoordinate + HEIGHT_STATUS];
    else
        [self.sunTopConstraint setConstant:sunYCoordinate];
    
    CGFloat rotationAngle = (CircleAngle / (100+HEIGHT_STATUS)) * shiftInPercents;
    self.sunImageView.transform = CGAffineTransformMakeRotation(DEGREES_TO_RADIANS(rotationAngle));
}

-(CGFloat)shiftInPercents{
    
    return ((DefaultHeight+HEIGHT_STATUS) / (100+HEIGHT_STATUS)) * -self.scrollView.contentOffset.y;
}

-(void)setupSkyPosition{
    
    CGFloat shiftInPercents = [self shiftInPercents];
    CGFloat skyTopConstant = (SkyDefaultShift - HEIGHT_STATUS) + ((SkyTopShift / (100+HEIGHT_STATUS)) * shiftInPercents) +  HEIGHT_STATUS;
...
-(void)scaleItems{
    ...
    CGFloat shiftInPercents = [self shiftInPercents];
    CGFloat buildigsScaleRatio = shiftInPercents / (100+HEIGHT_STATUS);
    
    if(buildigsScaleRatio <= BuildingsMaximumScale){
        
        CGFloat extraOffset = ABS(self.scrollView.contentOffset.y) - DefaultHeight - HEIGHT_STATUS;
        self.buildingsTopConstraint.constant = HEIGHT_STATUS + 8;
        if(DEVICE_SCREEN_INCH_5_8)
            self.buildingsHeightConstraint.constant = BuildingDefaultHeight + extraOffset + HEIGHT_STATUS - 25;
        else
            self.buildingsHeightConstraint.constant = BuildingDefaultHeight + extraOffset + HEIGHT_STATUS;
