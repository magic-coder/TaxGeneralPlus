...
- (void)viewDidLoad {
    ...
    //添加导航栏右侧按钮
    UIBarButtonItem *rightButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"push" style:UIBarButtonItemStylePlain target:self action:@selector(pushView:)];
    self.navigationItem.rightBarButtonItem = rightButtonItem;
    
    self.view.backgroundColor = RGBCOLOR(239, 239, 239);
}

- (void)pushView:(UIBarButtonItem *)sender {
...
- (void)initalizeAppearance
{
    ...
    for (int i = 0; i < _textArray.count + 1; i++) {
        CGRect frame = CGRectMake((i % 4) * (itemWidth + 10) +10, 88+35 + (i / 4) * 40 + 5, itemWidth, 30);
...
- (void)initMenuButton:(NSArray *)array
{
    ...
    for (int i = 0; i < array.count; i ++) {
        ...
        menuButton.tag = i;
        menuButton.frame = CGRectMake((i%4)*(buttonWidth+10)+10,88+35+(i/4)*40+5,buttonWidth,30);
