...
-(instancetype)initWithDependencyView:(UIView *)dependencyView MenuView:(UIView *)leftmenuView isShowCoverView:(BOOL)isCover{
    ...
    if(self = [super init]){
        ...
        self.coverViewframe = CGRectMake(self.menuViewframe.size.width, self.menuViewframe.origin.y, WIDTH_SCREEN - self.menuViewframe.size.width, self.menuViewframe.size.height);
    }
    return self;
}
...
-(void)show{
    ...
    // 调用即将显示代理方法
    if([self.delegate respondsToSelector:@selector(willAppear)]){
        [self.delegate willAppear];
    }
    ...
    self.coverView.frame = CGRectMake(0, 0, WIDTH_SCREEN, self.menuViewframe.size.height);
    ...
    if(UIGestureRecognizerStateBegan == gesture.state ||
       UIGestureRecognizerStateChanged == gesture.state){
        ...
        if(translation.x <= self.menuViewframe.size.width){// && self.leftMenuView.frame.origin.x != 0
            ...
            if(translation.x <= 10){
                self.coverView.frame = CGRectMake(0, self.menuViewframe.origin.y, WIDTH_SCREEN, self.menuViewframe.size.height);
            }
            ...
            self.coverView.frame    = CGRectMake(self.leftMenuView.frame.size.width+x, 0,WIDTH_SCREEN-self.leftMenuView.frame.size.width-x, h);
    ...
    if(UIGestureRecognizerStateBegan == recognizer.state ||
       UIGestureRecognizerStateChanged == recognizer.state){
        ...
        if(Place <= self.leftMenuView.frame.size.width &&  Place >0){
            x  = 0 - Place;
            self.coverView.frame    = CGRectMake(self.leftMenuView.frame.size.width-Place, 0,WIDTH_SCREEN-self.leftMenuView.frame.size.width+Place, h);
            self.coverView.alpha    = CoverViewAlpha*((w - Place) / w);
        }else if(Place >0){
             x  = - self.menuViewframe.size.width;
            self.coverView.frame    = CGRectMake(0, 0,WIDTH_SCREEN,h);
        }else{
            x = 0;
            self.coverView.frame    = CGRectMake(self.leftMenuView.frame.size.width, 0,WIDTH_SCREEN-self.leftMenuView.frame.size.width, h);
            self.coverView.alpha    = CoverViewAlpha;
        }
        self.leftMenuView.frame = CGRectMake(x, y, w, h);
    }else{
...
-(void)coverTap{
    ...
    // 调用即将隐藏代理方法
    if([self.delegate respondsToSelector:@selector(willDisappear)]){
    ...
    [UIView animateKeyframesWithDuration:0.3 delay:0 options:UIViewKeyframeAnimationOptionLayoutSubviews animations:^{
        ...
        self.coverView.frame    = CGRectMake(0, 0, WIDTH_SCREEN, HEIGHT_SCREEN);
...
-(void)removeCoverAndMenuView{
    ...
    self.leftMenuView.frame = CGRectMake(-self.leftMenuView.frame.size.width, 0, self.leftMenuView.frame.size.width, HEIGHT_SCREEN);
    self.coverView.frame    = CGRectMake(0, 0,WIDTH_SCREEN, HEIGHT_SCREEN);
    self.coverView.alpha    = 0.0;
